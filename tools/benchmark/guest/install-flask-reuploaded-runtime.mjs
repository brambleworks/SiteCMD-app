...
if (digest(manifest.requirements) !== manifest.requirementsSha256)
  throw new Error("Flask-Reuploaded runtime requirements differ from their manifest");
...
try {
  ...
  write("project/requirements.in", manifest.requirements);
  ...
  const python = `${root}/python/bin/python3.13`;
  ...
  if (lockedRequirementsSha256 !== manifest.lockSha256)
    throw new Error("Resolved Flask-Reuploaded dependencies differ from the frozen lock");
  const installArgs = [
    "pip",
    "install",
    "--python",
  ...
  console.log(JSON.stringify(verifyFlaskReuploadedRuntime(runtime)));
} catch (error) {
